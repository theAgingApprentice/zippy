...
/**
 * @brief Display the bootup screen.
 * ==========================================================================*/
void displayBootScreen() 
{
   int8_t row0 = 0; // First row of LCD.
   int8_t row1 = 1; // Second row of LCD.
   Log.traceln("<displayBootScreen> Display splash screen on LCD.");
   String bootMsg = "Booting Up";
   placeTextHcentre(HOST_NAME_PREFIX, row0);
   placeTextHcentre(bootMsg, row1);
} // displayBootScreen()
...
void startLcd() 
{
   ...
   lcd.init(G_I2C_BUS0_SDA, G_I2C_BUS0_SCL); // initialize the lcd 
