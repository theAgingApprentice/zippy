...
void setupLimitSwitches()
{
   ...
   pinMode(G_FRONT_BUMPER, INPUT_PULLUP); // Set GPIO pin to input.
   pinMode(G_BACK_BUMPER, INPUT_PULLUP); // Set GPIO pin to input.
} // setupLimitSwitches()
...
{
   if((digitalRead(G_FRONT_BUMPER) == false)) // Front limit switch pressed?
   {
   ...
   if(digitalRead(G_BACK_BUMPER) == false) // Back limit switch pressed?
   {
